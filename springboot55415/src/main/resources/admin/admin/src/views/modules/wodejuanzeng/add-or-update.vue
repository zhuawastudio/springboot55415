  ...
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.juanzengbianhao" label="捐赠编号" prop="juanzengbianhao">
              <el-input v-model="ruleForm.juanzengbianhao" 
                placeholder="捐赠编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="捐赠金额" prop="juanzengjine">
          <el-select :disabled="ro.juanzengjine" v-model="ruleForm.juanzengjine" placeholder="请选择捐赠金额">
            <el-option
                v-for="(item,index) in juanzengjineOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      this.$http({
		...
		if(json.biyenianfen!=''&&json.biyenianfen){
          ...
          this.$message.error(data.msg);
            ...
            this.juanzengjineOptions = "500,1000,2000,5000,10000".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `wodejuanzeng/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wodejuanzeng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wodejuanzengCrossAddOrUpdateFlag = false;
						 this.parent.search();
