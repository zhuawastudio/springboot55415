  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.biyenianfen!=''&&json.biyenianfen){
                this.ruleForm.biyenianfen = json.biyenianfen
	    		this.ro.biyenianfen = true;
		}
        } else {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaruxiaoyouhui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaruxiaoyouhuiCrossAddOrUpdateFlag = false;
						 this.parent.search();
